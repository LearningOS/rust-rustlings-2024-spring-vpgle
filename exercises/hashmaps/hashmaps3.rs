...
fn build_scores_table(results: String) -> HashMap<String, Team> {
    ...
    for r in results.lines() {
        ...
        let team_1_name = v[0].to_string();     // Germany
        let team_1_score: u8 = v[2].parse().unwrap(); // 2
        let team_2_name = v[1].to_string();     // England
        let team_2_score: u8 = v[3].parse().unwrap(); // 1
        ...
        if !scores.contains_key(&team_1_name) && !scores.contains_key(&team_2_name)  {
            ...
            scores.insert(team_1_name.clone(), Team{ goals_scored: scores.get(&team_1_name).unwrap().goals_scored + team_1_score ,
                                                 goals_conceded: scores.get(&team_1_name).unwrap().goals_conceded + team_2_score ,});
            scores.insert(team_2_name.clone(), Team{ goals_scored: scores.get(&team_2_name).unwrap().goals_scored + team_2_score ,
