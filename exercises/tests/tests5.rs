...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The provided `address` is guaranteed to be a valid
    // memory address pointing to a mutable `u32` value.
    let ptr = address as *mut u32;

    // SAFETY: We have ensured that `ptr` is a valid pointer to a `u32`.
    *ptr = 0xAABBCCDD;
}
